...
void InstantTalkWidget::hideAcceptButton(){
	//acceptButton.set_sensitive( false );
}

bool InstantTalkWidget::handleCommand( CommandString command ){
#ifdef DEBUG_OUTPUT
...
void InstantTalkWidget::startRinging(){
	if(!bell){
		bell = new Bell();
	}
	bell->start();
...
void InstantTalkWidget::stopRinging(){
	if (bell){
		bell->stop();
		bell=NULL;
	}
...
void InstantTalkWidget::dtmfPressed( uint8_t symbol ){
	char s[2];
	s[0]=symbol;
	s[1]=0;
	// Handle 0 <= symbol <= 11
...
string InstantTalkWidget::getMainCallId(){
	return mainCallId;
}

bool InstantTalkWidget::handlesCallId( string callId ){
...
#endif
void InstantTalkWidget::activeWidgetChanged( bool isActive, int currentActive ) {

	if( isActive == activeInstantTalkWidget ) {
#ifdef DEBUG_OUTPUT
// 		cerr << "CallWidget::activeCall - nothing to do here (no active state change)" << endl;
#endif
		return;
	} else {
		activeInstantTalkWidget = isActive;
	}
